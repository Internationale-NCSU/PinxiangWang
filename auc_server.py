...
def bidding_handler():
    ...
    try:
        ...
        if highest_bid >= lowest_price:
            ...
            # send ip to winner and seller
            winner_client.send(b'the Seller`s IP is ' + str(seller_client.getpeername()[0]).encode())
            seller_client.send(b'the Winning Buyer`s IP is ' + str(winner_client.getpeername()[0]).encode())
